namespace :zomato_dingtalk do
  namespace :deploy do
    ...
    task :updating do
      Capistrano::ZomatoDingtalk.new(self).run(:updating)
    end

    desc 'Notify about reverting deploy'
    task :reverting do
      Capistrano::ZomatoDingtalk.new(self).run(:reverting)
    end

    desc 'Notify about updated deploy'
    task :updated do
      Capistrano::ZomatoDingtalk.new(self).run(:updated)
    end

    desc 'Notify about reverted deploy'
    task :reverted do
      Capistrano::ZomatoDingtalk.new(self).run(:reverted)
    end

    desc 'Notify about failed deploy'
    task :failed do
      Capistrano::ZomatoDingtalk.new(self).run(:failed)
    end
